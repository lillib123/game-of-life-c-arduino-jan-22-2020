...
TEST(Life, live_cell_with_three_live_neighbors_lives)
{
    TEST_ASSERT_EQUAL_INT(1, rules(1, 3));
}

TEST(Life,live_cell_with_more_than_three_dies)
{
    TEST_ASSERT_EQUAL_INT(0,rules(1,4));
}

TEST(Life, dead_cell_with_exactly_three_live_neighbors_comes_to_life)
{
    TEST_ASSERT_EQUAL_INT(1, rules(0,3));
}

TEST(Life, dead_cell_with_two_live_neighbors_remains_dead)
{
    TEST_ASSERT_EQUAL_INT(0, rules(0,2));
}
TEST(Life, dead_cell_with_four_live_neighbors_remains_dead)
{
...
TEST_GROUP_RUNNER(Life)
{
    ...
    RUN_TEST_CASE(Life, live_cell_with_three_live_neighbors_lives)
    RUN_TEST_CASE(Life, live_cell_with_more_than_three_dies)
    RUN_TEST_CASE(Life, dead_cell_with_exactly_three_live_neighbors_comes_to_life)
    RUN_TEST_CASE(Life, dead_cell_with_two_live_neighbors_remains_dead)
    RUN_TEST_CASE(Life, dead_cell_with_four_live_neighbors_remains_dead)
}
